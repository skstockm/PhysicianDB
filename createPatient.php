<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "INSERT INTO patient (patient_name,patient_phone,patient_insurance,patient_addressNum,
										patient_city,patient_zip,patient_state) values(?, ?, ?, ?, ?, ?, ?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($name,$phone,$insurance,$addressNum,$patientCity,$zipCode,$state));
			Database::disconnect();
